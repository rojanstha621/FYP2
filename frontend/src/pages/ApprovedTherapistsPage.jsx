  ...
  return (
      <div className="max-w-7xl mx-auto px-4 py-6">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-3xl font-bold text-palette-dark">Approved Therapists</h1>
        </div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filtered.length ? (
              filtered.map((t) => (
                <div key={t.id} className="bg-palette-cream rounded-lg shadow-md hover:shadow-xl transition-shadow overflow-hidden border-l-4 border-palette-mauve">
                  {/* Therapist Avatar */}
                  <div className="bg-gradient-to-br from-blue-500 to-purple-600 h-32 flex items-center justify-center">
                    <div className="w-20 h-20 bg-palette-cream rounded-full flex items-center justify-center shadow-lg">
                      <span className="text-3xl font-bold text-palette-mauve">
                  ...
                  <div className="p-6">
                    ...
                    <div className="space-y-2 mb-4">
                      <div className="flex items-center text-sm text-palette-dark/70">
                        <svg className="w-4 h-4 mr-2 text-palette-dark/50" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <div className="flex gap-2">
                      <Link
                        to={`/therapists/${t.id}`}
                        className="flex-1 bg-palette-mauve hover:bg-palette-dark text-white py-2 px-4 rounded-md text-sm font-medium text-center transition-colors"
                      >
                        View Details
                      </Link>
                      {!assignedIds.has(String(t.id)) && (
                        <button
                          ...
                          className="flex-1 bg-green-600 hover:bg-green-700 disabled:bg-palette-dark/40 text-white py-2 px-4 rounded-md text-sm font-medium transition-colors"
                        >
                          {bookingLoading === t.id ? (
              ...
              <div className="col-span-full text-center py-12 bg-palette-cream rounded-lg shadow">
                ...
                <p className="mt-4 text-palette-dark/70">No therapists found.</p>
                <p className="text-sm text-palette-dark/60 mt-1">Try adjusting your search criteria.</p>
              </div>
            )}
          </div>
        )}
